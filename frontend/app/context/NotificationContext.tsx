...
function getDisplayGroupKey(n: WsNotification<any>): string {
  const t = String(n.type || '')
  const d: any = n.data || {}

  if (t.includes('Workflow')) {
    const instanceId = d?.instanceId || d?.instanceRecord?.instanceId || d?.instance_id || d?.id
    if (instanceId) return `workflow:${instanceId}`
    const templateId = d?.templateId || d?.instanceRecord?.templateId || ''
    const connectionId = d?.connectionId || d?.instanceRecord?.connectionId || ''
    return `workflow:${templateId}:${connectionId}`
  }

  if (t === 'DidCommCredentialStateChanged' || t.includes('CredentialStateChanged')) {
    const record = d?.credentialRecord || d?.credentialExchangeRecord || d?.record || d
    const credId = record?.id || d?.id || ''
    const connectionId = record?.connectionId || d?.connectionId || ''
    return `credential:${credId || connectionId}`
  }

  if (t === 'ProofStateChanged' || t.includes('ProofStateChanged')) {
    const record = d?.proofRecord || d?.record || d
    const proofId = record?.id || d?.id || ''
    const connectionId = record?.connectionId || d?.connectionId || ''
    return `proof:${proofId || connectionId}`
  }
...
export function NotificationProvider({ children }: { children: React.ReactNode }) {
  ...
  const displayNotifications = useMemo(
    () => {
      const grouped: WsNotification<any>[] = []
      const seen = new Set<string>()
      for (const n of items) {
        if (SILENT_TYPES.has(String(n.type))) continue
        const key = getDisplayGroupKey(n)
