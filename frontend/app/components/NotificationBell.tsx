...
function formatItem(
  ...
  if (t.includes('Workflow')) {
    const state = d?.newState || d?.state || d?.instanceRecord?.state || ''
    const status = d?.newStatus || d?.status || d?.instanceRecord?.status || ''
    const templateId = d?.templateId || d?.instanceRecord?.templateId || d?.workflowId || ''
    const templateName = prettifyLabel(
      d?.workflowTitle || d?.templateTitle || d?.templateName || d?.workflowName || templateId || 'Workflow'
    )
    const instanceId = d?.instanceId || d?.instanceRecord?.instanceId || ''
    const connectionLabel = d?.connectionLabel || (connId ? resolveConnectionName(connId) : '')
    const version = d?.templateVersion || d?.workflowVersion || ''
    const title = status === 'completed'
      ? 'Workflow completed'
      : state === 'pending_review'
        ? 'Workflow awaiting review'
        : status === 'waiting' || state === 'waiting_for_input'
          ? 'Workflow needs input'
          : 'Workflow updated'
    const body = [state || status, instanceId ? `#${String(instanceId).slice(-6)}` : ''].filter(Boolean).join(' · ')
    return {
      category: 'Workflow', iconName: 'workflow', iconColor: 'oklch(0.60 0.18 30)',
      title, body, from: connectionLabel || from, time, date, details: [
                  ...
                  >
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      {details.length > 0 && (
                        <div style={{ marginTop: 4, display: 'grid', gap: 2 }}>
                          {details.map((line) => (
                            <div key={line} style={{ fontSize: 10.5, color: 'var(--ink-3)', lineHeight: 1.35 }}>
                              {line}
                            </div>
                          ))}
                        </div>
                    ...
                    <button
                      ...
                      style={{
                        ...
                        color: 'var(--ink-3)', padding: '3px 6px', borderRadius: 4,
                      }}
                      onMouseEnter={(e) => { e.currentTarget.style.color = 'var(--ink)'; e.currentTarget.style.borderColor = 'var(--ink-4)' }}
